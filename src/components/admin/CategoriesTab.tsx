...
import { btnGhost, btnLight, input, Label } from './ui';
...
export default function CategoriesTab({
  ...
  async function seed() {
    ...
    await reload();
    setRows(
      CATEGORIES.map((c, i) => ({ slug: c.slug, name: c.name, descr: c.desc, photo: c.photo, sort: i }))
    );
    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        {[...rows]
          ...
          .map((row) => (
            <div key={row.slug} className="border border-white/12 p-5">
              <div className="grid md:grid-cols-[200px_1fr] gap-5">
                <div>
                  <div
                    className="aspect-[4/3] bg-cover bg-center border border-white/10 mb-3"
                  ...
                  <select
                    ...
                    disabled={dbEmpty}
                    className="w-full bg-ink border border-white/15 px-2 py-2 text-sm mb-2"
                  >
                    {STOCK_PHOTOS.map((p) => (
                  ...
                  <input
                    ...
                    disabled={dbEmpty}
                    className={`${input} text-[11px] font-mono text-paper/60 border-white/10`}
                  />
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    disabled={dbEmpty}
                    className={`${input} border-white/10 text-base mb-4`}
                  />
                  ...
                  <textarea
                    ...
                    disabled={dbEmpty}
                    className={`${input} border-white/10 resize-none flex-1`}
                  />
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    ...
                    <button
                      onClick={() => remove(row)}
                      disabled={busy || dbEmpty}
                      className="p-2 text-paper/40 hover:text-signal ml-auto"
                      aria-label="Удалить"
                    >
      ...
      <p className="mt-6 text-[12px] text-paper/40 leading-relaxed max-w-xl">
